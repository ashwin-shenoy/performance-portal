...
const UploadedFiles = () => {
  ...
  const downloadReportFile = async (reportId, fallbackName = null) => {
    ...
    const resolvedName = matchedName || fallbackName || `report-${reportId}`;
  ...
  const handleGenerateReport = async (record) => {
    try {
      ...
      const reportId = response.data?.data?.reportId || response.data?.reportId;
      const fileName = response.data?.data?.fileName || null;
      if (reportId) {
        ...
        {selectedTest && (
          <StructuredListWrapper>
            <StructuredListBody>
              ...
              <StructuredListRow>
                <StructuredListCell head>Test Start Time</StructuredListCell>
                <StructuredListCell>
                  {selectedTest.testStartTime ? dayjs(selectedTest.testStartTime).format('YYYY-MM-DD HH:mm:ss') : 'N/A'}
                </StructuredListCell>
              </StructuredListRow>
              <StructuredListRow>
                <StructuredListCell head>Test End Time</StructuredListCell>
                <StructuredListCell>
                  {selectedTest.testEndTime ? dayjs(selectedTest.testEndTime).format('YYYY-MM-DD HH:mm:ss') : 'N/A'}
                </StructuredListCell>
              </StructuredListRow>
              <StructuredListRow>
              ...
              <StructuredListRow>
                <StructuredListCell head>Pod CPU (avg / max)</StructuredListCell>
                <StructuredListCell>
                  {selectedTest.podCpuAvg != null || selectedTest.podCpuMax != null
                    ? `${selectedTest.podCpuAvg ?? 'N/A'} / ${selectedTest.podCpuMax ?? 'N/A'}`
                    : 'N/A'}
                </StructuredListCell>
              </StructuredListRow>
              <StructuredListRow>
                <StructuredListCell head>Pod Memory (avg / max)</StructuredListCell>
                <StructuredListCell>
                  {selectedTest.podMemoryAvg != null || selectedTest.podMemoryMax != null
                    ? `${selectedTest.podMemoryAvg ?? 'N/A'} / ${selectedTest.podMemoryMax ?? 'N/A'}`
                    : 'N/A'}
                </StructuredListCell>
              </StructuredListRow>
              <StructuredListRow>
                <StructuredListCell head>JVM Heap Used % (avg)</StructuredListCell>
                <StructuredListCell>{selectedTest.jvmHeapUsedPercentAvg ?? 'N/A'}</StructuredListCell>
              </StructuredListRow>
              <StructuredListRow>
                <StructuredListCell head>JVM GC Pause ms (P95)</StructuredListCell>
                <StructuredListCell>{selectedTest.jvmGcPauseMsP95 ?? 'N/A'}</StructuredListCell>
              </StructuredListRow>
              <StructuredListRow>
                <StructuredListCell head>JVM Process CPU (avg)</StructuredListCell>
                <StructuredListCell>{selectedTest.jvmProcessCpuAvg ?? 'N/A'}</StructuredListCell>
              </StructuredListRow>
