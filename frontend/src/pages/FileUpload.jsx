...
const FileUpload = () => {
  ...
  const [latestTestRunData, setLatestTestRunData] = useState(null);
  ...
  useEffect(() => {
    ...
    const pollStatus = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setLatestTestRunData(response.data || null);
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      try {
        await axios.get(API_ENDPOINTS.REPORT_VALIDATE_JMETER(latestTestRunId));
  ...
  const handleUpload = async () => {
    ...
    if (formData.testStartTime) {
      formDataToSend.append('testStartTime', dayjs(formData.testStartTime).format('YYYY-MM-DDTHH:mm:ss'));
    }
    if (formData.testEndTime) {
      formDataToSend.append('testEndTime', dayjs(formData.testEndTime).format('YYYY-MM-DDTHH:mm:ss'));
    }
          ...
          <Grid narrow>
            ...
            <Column sm={4} md={8} lg={8}>
              <TextInput
                id="test-start-time"
                type="datetime-local"
                labelText="Test Start Time (Optional Override)"
            ...
            <Column sm={4} md={8} lg={8}>
              <TextInput
                id="test-end-time"
                type="datetime-local"
                labelText="Test End Time (Optional Override)"
            ...
            <Column sm={4} md={8} lg={16}>
              ...
              {latestTestRunData && testRunStatus === 'COMPLETED' && (
                <Tile style={{ marginTop: '1rem', padding: '1rem', background: '#f4f4f4' }}>
                  <p style={{ margin: '0 0 0.5rem 0', fontWeight: 600 }}>Fetched Runtime Metrics</p>
                  <p style={{ margin: '0.25rem 0' }}>
                    Test Window: {latestTestRunData.testStartTime ? dayjs(latestTestRunData.testStartTime).format('YYYY-MM-DD HH:mm:ss') : 'N/A'}
                    {' '}→{' '}
                    {latestTestRunData.testEndTime ? dayjs(latestTestRunData.testEndTime).format('YYYY-MM-DD HH:mm:ss') : 'N/A'}
                  </p>
                  <p style={{ margin: '0.25rem 0' }}>
                    Pod CPU (avg/max): {latestTestRunData.podCpuAvg ?? 'N/A'} / {latestTestRunData.podCpuMax ?? 'N/A'}
                  </p>
                  <p style={{ margin: '0.25rem 0' }}>
                    Pod Memory (avg/max): {latestTestRunData.podMemoryAvg ?? 'N/A'} / {latestTestRunData.podMemoryMax ?? 'N/A'}
                  </p>
                  <p style={{ margin: '0.25rem 0' }}>
                    JVM Heap Used % (avg): {latestTestRunData.jvmHeapUsedPercentAvg ?? 'N/A'}
                  </p>
                  <p style={{ margin: '0.25rem 0' }}>
                    JVM GC Pause ms (P95): {latestTestRunData.jvmGcPauseMsP95 ?? 'N/A'}
                  </p>
                  <p style={{ margin: '0.25rem 0' }}>
                    JVM Process CPU (avg): {latestTestRunData.jvmProcessCpuAvg ?? 'N/A'}
                  </p>
                </Tile>
              )}
            </Column>
