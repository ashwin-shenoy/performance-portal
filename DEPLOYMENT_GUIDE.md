...
### Secure npm Auth for Local Docker Builds (Optional)

Use this only when you build frontend images from source on server/local machine (not needed if you only pull prebuilt GHCR images).

```bash
cd /opt/performance-portal

# Create gitignored npmrc file from template
cp frontend/.npmrc.example frontend/.npmrc

# Set token for current shell only
export NPM_TOKEN=your-read-only-token
export DOCKER_BUILDKIT=1

# Build locally with secure secret mount
docker compose up -d --build
```

`frontend/.npmrc` format:
